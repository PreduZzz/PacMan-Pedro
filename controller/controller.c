...
void controller_handle_input(GameModel *m) {
    while (kbhit_custom()) {
        ...
        if (ch == 'd') { m->desired_dx = 1;  m->desired_dy = 0;  }
        if (ch == 'D') { m->debug_mode = !m->debug_mode; m->dijkstra_path_on = m->debug_mode; }
    ...
    while (kbhit_custom()) {
        ...
        if (ch == 0 || ch == 224) {
            ...
            if (arrow == 72) { /* up */
                ...
                m->map_selected = (m->map_selected - 1 + MAP_COUNT) % MAP_COUNT;
        ...
        if (ch == 27 && kbhit_custom()) {
            ...
            if (c2 == '[' && kbhit_custom()) {
                ...
                if (c3 == 'D') m->map_selected = (m->map_selected - 1 + MAP_COUNT) % MAP_COUNT;    /* left */
                if (c3 == 'C') m->map_selected = (m->map_selected + 1) % MAP_COUNT;                /* right */
            }
            continue;
        }
#endif
        ...
        if (ch == 'm' || ch == 'M') { m->map_selected = (m->map_selected + 1) % MAP_COUNT; continue; }
...
int main(void) {
    ...
    printf("Digite o nome do bruxo/jogador (ate 15 caracteres): ");
    fflush(stdout);
    if (fgets(model.player_name, sizeof(model.player_name), stdin)) {
        model.player_name[strcspn(model.player_name, "\r\n")] = '\0';
        if (!model.player_name[0]) strcpy(model.player_name, "BRUXO");
    }
    ...
    printf("\n");
    printf("  %s╔══════════════════════════════════════╗%s\n", CLR_WALL, CLR_RESET);
    printf("  %s║%s  %sObrigado por jogar PAC-MAN HOGWARTS!%s        %s║%s\n",
    ...
    model_save_scores(&model);
    model_print_final_report(&model);
    model_free(&model);
    view_cleanup();
